...
// 	});
    ...
    jQuery('#footer_credit_text').on('input',function(e){
	    jQuery('#preview').contents().find('#cspio-credit a').text(jQuery('#footer_credit_text').val());
	});
    ...
    jQuery('.button_color_picker').colorpicker({ component: '.form-control, .add-on, .input-group-addon' }).on('changeColor.colorpicker', function(event){
      ...
      jQuery('#preview').contents().find('#cspio-subscribe-btn,.cspio .progress-bar, a.btn-primary').css('background-color',"rgba("+event.color.toRGB().r+","+event.color.toRGB().g+","+event.color.toRGB().b+","+event.color.toRGB().a+")");
      ...
      if(lightness >= 0.65){
        var color = '#000';
        jQuery('#preview').contents().find('#cspio-subscribe-btn,.cspio .progress-bar span,a.btn-primary').css('color','#000');
      }else{
        var color = '#fff';
        jQuery('#preview').contents().find('#cspio-subscribe-btn,.cspio .progress-bar span,a.btn-primary').css('color','#fff');
      }
    ...
    jQuery('.form_color_picker').colorpicker({ component: '.form-control, .add-on, .input-group-addon' }).on('changeColor.colorpicker', function(event){
      jQuery('#preview').contents().find('input,.progress').css('background-color',"rgba("+event.color.toRGB().r+","+event.color.toRGB().g+","+event.color.toRGB().b+","+event.color.toRGB().a+")");
    ...
    //Fraud Detection
    if (jQuery('#enable_reflink').is(':checked')) {
        jQuery("#enable_fraud_detection").parent().show();
    }else{
        jQuery("#enable_fraud_detection").parent().hide();
    }
    

    jQuery("#enable_reflink").change(function() {
    if(this.checked) {
        jQuery("#enable_fraud_detection").parent().fadeIn();
    }else{
        jQuery("#enable_fraud_detection").parent().hide();
    }
    });
...
jQuery(document).ready(function() {
   ...
   jQuery('#refresh_page').on('click',function(e){
	...
	jQuery("#enable_reflink,#credit_type,#credit_position,#enable_fitvid,#enable_retinajs,#enable_recaptcha,#enable_wp_head_footer,#bg_video_audio, #bg_video_loop,#display_name,#bg_slideshow_slide_transition,#progressbar_effect,#container_flat").change(function() {
	    save_page();
	});
