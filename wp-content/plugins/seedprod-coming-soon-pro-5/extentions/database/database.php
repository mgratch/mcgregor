...
add_action('seed_cspv5_emaillist_database', 'seed_cspv5_emaillist_database_add_subscriber');

function seed_cspv5_emaillist_database_add_subscriber($args){
        ...
        if(isset($insert_result) && $insert_result != false){
			...
			$seed_cspv5_post_result['ref_url'] = seed_cspv5_ref_link();
                global $wpdb;
                $tablename = $wpdb->prefix . SEED_CSPV5_SUBSCRIBERS_TABLENAME;
                // Update Row
                $r = $wpdb->update( 
            ...
            if((!empty($enable_reflink) && $enable_reflink) && !empty($seed_cspv5_post_result['ref'])){
                $rf_url = "<br><br>".$txt_stats_referral_url.'<br>'.seed_cspv5_ref_link();
...
function seed_cspv5_section_database($emaillist,$page_id) {
    // Get settings
    $settings_name = 'seed_cspv5_'.$page_id.'_'.$emaillist;
