...
function seed_cspv5_get_page_default_settings(){
    ...
    $settings = seed_cspv5_array_add($settings,'txt_stats_referral_subscribers','Conversion');
...
function seed_cspv5_ref_link(){
    ...
    if(!empty($seed_cspv5_post_result['ref'])){
        ...
        if(!empty($ref_url)){
            ...
            $ref_link = 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}".'?ref='.$seed_cspv5_post_result['ref'];
        }
    }else{
