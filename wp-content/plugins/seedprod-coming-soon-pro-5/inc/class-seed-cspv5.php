...
class SEED_CSPV5{
    ...
    function render_comingsoon_page() {
            ...
            if(!empty($client_view_url)){
                ...
                if((strtolower(basename($_SERVER['REQUEST_URI'])) == trim(strtolower($client_view_url))) || (strtolower($_GET['bypass']) == trim(strtolower($client_view_url)))) {
                    if(!empty($bypass_expires)){
                        $exipres_in = time()+$bypass_expires;
                    }else{
        ...
        if(!empty($client_view_url)){
            ...
            if((strtolower(basename($_SERVER['REQUEST_URI'])) == trim(strtolower($client_view_url))) || (strtolower($_GET['bypass']) == trim(strtolower($client_view_url)))) {
                ...
                if(!empty($bypass_expires)){
                    ...
                    $seed_cspv5_bypass_expires = $bypass_expires;
                }
        ...
        if(!empty($include_exclude_options) && $include_exclude_options == '1'  && @preg_match("/^\/$|\?(.)*/",$_SERVER['REQUEST_URI']) == 0 && $is_preview == false){
            return false;
        }
