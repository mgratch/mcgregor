...
sRangeFormat=aoColumn.sRangeFormat;else sRangeFormat=properties.sRangeFormat;if(aoColumn!==null){switch(aoColumn.type){case 'null':break;case 'number':wdtCreateInput(oTable,aoColumn,columnIndex,sColumnLabel,th);break;case 'number-range':wdtCreateNumberRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes);break;case 'date-range':wdtCreateDateRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes);break;case 'datetime-range':wdtCreateDateTimeRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes);break;case 'time-range':wdtCreateTimeRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes);break;case 'select':wdtCreateSelectbox(oTable,aoColumn,columnIndex,sColumnLabel,th);break;case 'multiselect':wdtCreateMultiSelectbox(oTable,aoColumn,columnIndex,sColumnLabel,th);break;case 'checkbox':wdtCreateCheckbox(oTable,aoColumn,columnIndex,sColumnLabel,th);break;case 'text':default:wdtCreateInput(oTable,aoColumn,columnIndex,sColumnLabel,th);break}}});for(var j=0;j<customSearchIndexes.length;j++){var fnSearch_=function(){var id=oTable.attr("id");if((typeof $("#"+id+"_range_from_"+customSearchIndexes[j]).val()==='undefined')||(typeof $("#"+id+"_range_to_"+customSearchIndexes[j]).val()==='undefined')){return properties.sRangeSeparator}
...
var tableId=oTable.attr('id');var search='';if(aoColumn.exactFiltering){search=serverSide?value:"^"+value+"$";oTable.api().column(columnIndex).search(value?search:'',!0,!1)}else{search=bIsNumber&&!serverSide?'^'+value:value;oTable.api().column(columnIndex).search(search,bIsNumber,!1)}
if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}
if(aoColumn.defaultValue){var defaultValue=jQuery.isArray(aoColumn.defaultValue)?aoColumn.defaultValue[0]:aoColumn.defaultValue;jQuery(input).val(defaultValue);oTable.fnFilter(defaultValue,columnIndex)}}
function wdtCreateNumberRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes){var tableId=oTable.attr('id');var fromDefaultValue='',toDefaultValue='',defaultValue=aoColumn.defaultValue;var tableDescription=jQuery.parseJSON(jQuery('#'+oTable.data('described-by')).val());var numberFormat=(typeof tableDescription.numberFormat!=='undefined')?parseInt(tableDescription.numberFormat):1;var replaceFormat=numberFormat===1?/\./g : /,/g;if(defaultValue!==''){fromDefaultValue=defaultValue[0];toDefaultValue=defaultValue[1]}
...
if(iMin!==''){iMin=iMin*1}
if(iMax!==''){iMax=iMax*1}
iValue=iValue*1;return(iMin===""&&iMax==="")||(iMin===""&&iValue<=iMax)||(iMin<=iValue&&""===iMax)||(iMin<=iValue&&iValue<=iMax)});jQuery('#'+sFromId+', #'+sToId,th).keyup(function(){numberRangeSearch()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);jQuery(document).ready(function(){jQuery(from).keyup()})}
if(toDefaultValue){jQuery(to).val(toDefaultValue);jQuery(document).ready(function(){jQuery(to).keyup()})}
...
fnOnFiltered()}}
function wdtCreateDateRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes){var tableId=oTable.attr('id');var fromDefaultValue='',toDefaultValue='',defaultValue=aoColumn.defaultValue;var dateFormat=getMomentWdtDateFormat();if(defaultValue!==''){fromDefaultValue=defaultValue[0];toDefaultValue=defaultValue[1]}
...
return!1;return(isNaN(dStartDate.getTime())&&dCellDate<=dEndDate)||(dStartDate<=dCellDate&&isNaN(dEndDate.getTime()))||(dStartDate<=dCellDate&&dCellDate<=dEndDate)});jQuery('#'+sFromId+', #'+sToId,th).on('dp.change',function(){if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);jQuery(document).ready(function(){jQuery(from).change()})}
if(toDefaultValue){jQuery(to).val(toDefaultValue);jQuery(document).ready(function(){jQuery(to).change()})}}
...
fnOnFiltered()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);jQuery(document).ready(function(){jQuery(from).change()})}
if(toDefaultValue){jQuery(to).val(toDefaultValue);jQuery(document).ready(function(){jQuery(to).change()})}}
function wdtCreateTimeRangeInput(oTable,aoColumn,columnIndex,sColumnLabel,th,customSearchIndexes){var tableId=oTable.attr('id');var fromDefaultValue='',toDefaultValue='',defaultValue=aoColumn.defaultValue;var timeFormat=getMomentWdtTimeFormat();if(defaultValue!==''){fromDefaultValue=defaultValue[0];toDefaultValue=defaultValue[1]}
...
return!1;return(isNaN(dStartTime.getTime())&&dCellTime<=dEndTime)||(dStartTime<=dCellTime&&isNaN(dEndTime.getTime()))||(dStartTime<=dCellTime&&dCellTime<=dEndTime)});jQuery('#'+sFromId+', #'+sToId,th).on('dp.change',function(){if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()});if(fromDefaultValue){jQuery(from).val(fromDefaultValue);jQuery(document).ready(function(){jQuery(from).change()})}
if(toDefaultValue){jQuery(to).val(toDefaultValue);jQuery(document).ready(function(){jQuery(to).change()})}}
function wdtCreateSelectbox(oTable,aoColumn,columnIndex,sColumnLabel,th){var tableId=oTable.attr('id'),serverSide=oTable.fnSettings().oFeatures.bServerSide,selected;var dateFormat=getMomentWdtDateFormat();var timeFormat=getMomentWdtTimeFormat();if(aoColumn.values===null)
...
var selectTitle=aoColumn.filterLabel?aoColumn.filterLabel:'';var select='<select class="wdt-select-filter wdt-filter-control selectpicker" title="'+selectTitle+'" data-index="'+columnIndex+'"><option value="">'+' '+'</option>';var iLen=aoColumn.values.length;for(var j=0;j<iLen;j++){if(typeof(aoColumn.values[j])!=='object'){selected='';if((aoColumn.defaultValue!=='')&&(encodeURI(aoColumn.values[j])==encodeURI(aoColumn.defaultValue))){selected='selected="selected" '}
var optionLabel=aoColumn.values[j];if(aoColumn.values[j]==='possibleValuesAddEmpty'){optionLabel=' '}
if(serverSide){if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='date'){optionLabel=moment(optionLabel,moment.ISO_8601).format(dateFormat)}
if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='time'){optionLabel=moment(optionLabel,'hh:mm:ss').format(timeFormat.replace('h','hh'))}
if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='datetime'){optionLabel=moment(optionLabel,moment.ISO_8601).format(dateFormat+' '+timeFormat.replace('h','hh'))}}
select+='<option '+selected+' value="'+encodeURI(aoColumn.values[j])+'">'+optionLabel+'</option>'}else{selected='';if((aoColumn.defaultValue!=='')&&(aoColumn.values[j].value==aoColumn.defaultValue)){selected='selected="selected" '}
select+='<option '+selected+'value="'+encodeURI(aoColumn.values[j].value)+'">'+aoColumn.values[j].label+'</option>'}}
select=jQuery(select+'</select>');th.html(select);th.wrapInner('<span class="filter_column filter_select" data-filter_type="selectbox" data-index="'+columnIndex+'"/>');select.on('change.selectChange',function(){selectboxSearch.call(jQuery(this))});if(aoColumn.defaultValue){oTable.fnFilter(aoColumn.defaultValue,columnIndex)}
jQuery('.selectpicker[data-index='+columnIndex+']').selectpicker('refresh');function selectboxSearch(){var search='';if(jQuery(this).val()==='possibleValuesAddEmpty'&&!serverSide){oTable.api().column(columnIndex).search('^$',!0,!1)}else{if(aoColumn.exactFiltering){search=serverSide?decodeURIComponent(jQuery(this).val()):'^'+decodeURIComponent(jQuery(this).val())+'$';oTable.api().column(columnIndex).search(jQuery(this).val()?search:'',!0,!1)}else{oTable.api().column(columnIndex).search(decodeURIComponent(jQuery(this).val()),!0,!1)}}
if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}
function wdtCreateMultiSelectbox(oTable,aoColumn,columnIndex,sColumnLabel,th){var tableId=oTable.attr('id'),serverSide=oTable.fnSettings().oFeatures.bServerSide,selected;var dateFormat=getMomentWdtDateFormat();var timeFormat=getMomentWdtTimeFormat();if(!jQuery.isArray(aoColumn.defaultValue)){aoColumn.defaultValue=[aoColumn.defaultValue]}
if(aoColumn.values===null)
aoColumn.values=getColumnDistinctValues(tableId,columnIndex,!1);var selectTitle=aoColumn.filterLabel?aoColumn.filterLabel:'';var select='<select class="wdt-multiselect-filter wdt-filter-control selectpicker" title="'+selectTitle+'" data-index="'+columnIndex+'" multiple>';var iLen=aoColumn.values.length;for(var j=0;j<iLen;j++){selected=jQuery.inArray(aoColumn.values[j].toString(),aoColumn.defaultValue)!==-1?selected='selected="selected" ':'';if(typeof(aoColumn.values[j])!=='object'){var optionLabel=aoColumn.values[j];if(serverSide){if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='date'){optionLabel=moment(optionLabel,moment.ISO_8601).format(dateFormat)}
if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='time'){optionLabel=moment(optionLabel,'hh:mm:ss').format(timeFormat.replace('h','hh'))}
if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='datetime'){optionLabel=moment(optionLabel,moment.ISO_8601).format(dateFormat+' '+timeFormat.replace('h','hh'))}}
if(aoColumn.values[j]==='possibleValuesAddEmpty'){optionLabel=' '}
select+='<option '+selected+' value="'+encodeURI(aoColumn.values[j])+'">'+optionLabel+'</option>'}else{select+='<option '+selected+'value="'+encodeURI(aoColumn.values[j].value)+'">'+aoColumn.values[j].label+'</option>'}}
select=jQuery(select+'</select>');th.html(select);th.wrapInner('<span class="filter_column filter_select" data-filter_type="multiselectbox" data-index="'+columnIndex+'" />');select.change(function(){multiSelectboxSearch.call(jQuery(this))});if(aoColumn.defaultValue[0]){var search='';for(var i=0;i<aoColumn.defaultValue.length;i++){search+=buildSearchStringForMultiFilters(i,aoColumn.defaultValue[i],aoColumn.defaultValue.length,aoColumn.exactFiltering)}
oTable.fnFilter(search,columnIndex,!0,!1);fnOnFiltered()}
jQuery('.selectpicker[data-index='+columnIndex+']').selectpicker('refresh');function multiSelectboxSearch(){var search='',selectedOptions;selectedOptions=jQuery(this).selectpicker('val');if(selectedOptions!==null){var selectedOptionsLength=selectedOptions.length}
jQuery.each(selectedOptions,function(index,value){search+=buildSearchStringForMultiFilters(index,value,selectedOptionsLength,aoColumn.exactFiltering)});oTable.api().column(columnIndex).search(search,!0,!1);if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}
function wdtCreateCheckbox(oTable,aoColumn,columnIndex,sColumnLabel,th){var tableId=oTable.attr('id'),serverSide=oTable.fnSettings().oFeatures.bServerSide;var dateFormat=getMomentWdtDateFormat();var timeFormat=getMomentWdtTimeFormat();if(!jQuery.isArray(aoColumn.defaultValue)){aoColumn.defaultValue=[aoColumn.defaultValue]}
if(aoColumn.values===null)
aoColumn.values=getColumnDistinctValues(tableId,columnIndex,!1);var r='',j,iLen=aoColumn.values.length,dialogRender=!0;if(typeof aoColumn.sSelector!=='undefined'){dialogRender=aoColumn.checkboxesInModal}
...
r+='<div id="'+checkboxesDivId+'">';for(j=0;j<iLen;j++){if(aoColumn.values[j]!==null){var value=typeof aoColumn.values[j]!=='object'?aoColumn.values[j]:aoColumn.values[j].value;var label=typeof aoColumn.values[j]!=='object'?aoColumn.values[j]:aoColumn.values[j].label;var checked=jQuery.inArray(value.toString(),aoColumn.defaultValue)!==-1?'checked="checked" ':'';if(serverSide){if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='date'){label=moment(label,moment.ISO_8601).format(dateFormat)}
if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='time'){label=moment(label,'hh:mm:ss').format(timeFormat)}
if(oTable.fnSettings().aoColumns[columnIndex].wdtType==='datetime'){label=moment(label,moment.ISO_8601).format(dateFormat+' '+timeFormat.replace('h','hh'))}}
...
oTable.fnFilter('',$modal.attr('data-current-checkbox-dialog').substr($modal.attr('data-current-checkbox-dialog').length-1),!0,!1);fnOnFiltered()})})}
function checkboxSearch(columnIndex,checkboxesDivId){var search='',checkedInputs;checkedInputs=jQuery(this).closest('#'+checkboxesDivId).find('input:checkbox:checked');var checkedInputsLength=checkedInputs.length;jQuery.each(checkedInputs,function(index){search+=buildSearchStringForMultiFilters(index,jQuery(this).val(),checkedInputsLength,aoColumn.exactFiltering)});oTable.api().column(columnIndex).search(search,!0,!1);if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}
function getColumnDistinctValues(tableId,columnIndex,applySearch){applySearch=applySearch?'applied':'none';return wpDataTables[tableId].api().column(columnIndex,{search:applySearch}).data().unique().toArray().filter(Boolean).sort()}
function buildSearchStringForMultiFilters(index,value,valuesLength,exactFiltering){var search='',or='|';if((index===0&&valuesLength===1)||(index!==0&&index===valuesLength-1)){or=''}
search=search.replace(/^\s+|\s+$/g,'');if(exactFiltering){search=search+'^'+value.replace(/\+/g,'\\+')+'$'+or}else{search=search+value.replace(/\+/g,'\\+')+or}
...
wdtNotify(wpdatatables_edit_strings.success,wpdatatables_edit_strings.dataSaved,'success');setTimeout(function(){if(closeDialog){$('#wdt-frontend-modal').modal('hide')}else{$(tableDescription.selector+'_edit_dialog .editDialogInput').val('');$(tableDescription.selector+'_edit_dialog .editDialogInput').selectpicker('val','');$('.fileinput').removeClass('fileinput-exists').addClass('fileinput-new');$('.fileinput').find('div.fileinput-exists').removeClass('fileinput-exists').addClass('fileinput-new');$('.fileinput').find('.fileinput-filename').text('');$('.fileinput').find('.fileinput-preview').html('');if(tinymce.activeEditor)
