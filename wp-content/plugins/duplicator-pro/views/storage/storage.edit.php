...
if (isset($_REQUEST['action'])) {
	...
	if ($_REQUEST['action'] == 'save') {
		...
		// $dropbox_client = DUP_PRO_Storage_Entity::get_dropbox_client(false);
		// $access_token = $storage->get_dropbox_combined_access_token();
		// $dropbox_client->SetAccessToken($access_token);
...
<table id="provider-<?php echo DUP_PRO_Storage_Types::FTP ?>" class="form-table provider" >
	...
	<tr>
		...
		<td>
			...
			<p>
				<i><?php DUP_PRO_U::_e("Test connection by sending and receiving a small file to/from the account."); ?><br/>
				<?php DUP_PRO_U::_e("<b>Note:</b> This storage type only supports only FTP and SFTP, FTPS is not supported."); ?></i>
			</p>
		</td>
	</tr>
</table>	
...
<table id="provider-<?php echo DUP_PRO_Storage_Types::Local ?>" class="provider form-table">
	<tr valign="top">
		...
		<td>
			...
			<p>
				<i><?php DUP_PRO_U::_e("Where to store on the server, <b>not your development box.</b> On Linux servers start with '/' (e.g. /mypath). On Windows use drive letters (e.g. E:/mypath)."); ?></i>
			</p>
        ...
        DupPro.Storage.Dropbox.DropboxGetAuthUrl = function() 
        {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    {
                        alert("<?php DUP_PRO_U::_e('Error getting Dropbox authentication URL. Please try again later.') ?>");
                        jQuery('.authorization-state').show();
                    }
                },
                error: function (data) {
                    alert("<?php DUP_PRO_U::_e('Unable to get Dropbox authentication URL.') ?>");                   
                },
        ...
        DupPro.Storage.Dropbox.FinalizeSetup = function() 
        {
             if ($('#dropbox-auth-code').val().length > 5) {
                 $("#dup-storage-form").submit();
                  ...
                  alert("<?php DUP_PRO_U::_e('Please enter your Dropbox authorization code!') ?>");
             }
        }
