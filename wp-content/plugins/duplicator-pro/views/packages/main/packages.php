...
<style>
	...
	.remote-data {position:relative;}
	.remote-data:after {content:attr(data-badge); position:absolute; top:-5px; right:-8px; font-size:.5em; background:#8CB9E6; width:6px; height:6px; border-radius:50%;}
	...
	<tr>
		<td>
			...
			<?php if ($global->profile_alpha) : ?>
				<a href="admin.php?page=duplicator-pro-tools&tab=migrate" id="btn-logs-dialog" class="button" title="<?php DUP_PRO_U::_e("Import") ?>"><i class="fa fa-download"></i></a>
			<?php endif; ?>
		</td>
		<td>
			<div class="btnnav">
		...
		<tr class="dpro-pkinfo <?php echo $css_alt ?>" id="duppro-packagerow-<?php echo $row['id']; ?>">
			...
			<td class="get-btns">
				...
				<nav class="dpro-dnload-menu">
					<button <?php DUP_PRO_UI::echoDisabled(!$archive_exists) ?> class="dpro-dnload-menu-btn button no-select" type="button">
				...
				<nav class="dpro-bar-menu">
					<button type="button" class="dpro-store-btn button no-select dpro-bar-menu-btn " title="<?php DUP_PRO_U::_e("More Items") ?>">
						<i class="fa fa-bars <?php echo ($remote_display) ? 'remote-data' : '';?>"></i>
					</button>
					<nav class="dpro-bar-menu-items">
						...
						<?php if ($storage_problem) : ?>
							<div onclick="DupPro.Pack.ShowRemote(<?php echo "$Package->ID, '$Package->NameHash'"; ?>);" title="<?php DUP_PRO_U::_e("Storage download not accessible from here") ?>">
								<span><i class="fa fa-exclamation-triangle"></i> <?php DUP_PRO_U::_e("Storage") ?></span>
							</div>
		...
		<tr class="dpro-pkinfo  <?php echo $css_alt ?>" id="duppro-packagerow-<?php echo $row['id']; ?>">
			...
			<td class="get-btns" colspan="3">
				<?php if ($status >= 75) : ?>
					...
					<button type="button" class="dpro-store-btn button no-select" title="<?php DUP_PRO_U::_e("Package Details") ?>" onclick="DupPro.Pack.OpenPackDetail(<?php echo "$Package->ID"; ?>);">
						<i class="fa fa-archive" ></i>
					</button>
				<?php else : ?>
					<?php if ($status == 0): ?>
