...
EOT;
    ...
    //Check if there is need to replace collation

    if(!empty($_POST['dbcolsearchreplace']) && $dbTestIn->dbcollatefb){
        $collation_replace_list = json_decode(stripslashes($_POST['dbcolsearchreplace']),true);
        ...
        if($collation_replace_list !== NULL){
            foreach($collation_replace_list as $val){
                ...
                foreach($sql_result_file_data as $key=>$query){
                    $sql_result_file_data[$key] = str_replace($val['search'],$val['replace'],$query);
                    if($replace_charset){
                        $sql_result_file_data[$key] = str_replace('utf8mb4','utf8',$sql_result_file_data[$key]);
                    }
                }
                DUPX_Log::info("The collation {$val['search']} was replaced with {$val['replace']}\n",1);
            }
        }else{
            DUPX_Log::info("Can't decode collation replace list JSON.\n",1);
        }
	...
	DUPX_Log::info("--------------------------------------");
	DUPX_Log::info("MYSQL VERSION:\tThis Server: {$dbvar_version} -- Build Server: {$GLOBALS['DUPX_AC']->version_db}");
	...
	if (version_compare($dbvar_version, $GLOBALS['DUPX_AC']->version_db) < 0) {
		DUPX_Log::info("\nNOTICE: This servers version [{$dbvar_version}] is less than the build version [{$GLOBALS['DUPX_AC']->version_db}].  \n"
...
if ($_POST['dbaction'] == 'manual') {
	...
	if ($dbtable_count == 0) {
		DUPX_Log::info("NOTICE: You may have to manually run the installer-data.sql to validate data input. Also check to make sure your installer file is correct and the
			table prefix '{$GLOBALS['DUPX_AC']->wp_tableprefix}' is correct for this particular version of WordPress. \n");
	}
	...
	@mysqli_query($dbh, "DELETE FROM `{$GLOBALS['DUPX_AC']->wp_tableprefix}duplicator_pro_packages`");
	$dbdelete_count1 = @mysqli_affected_rows($dbh);

	@mysqli_query($dbh, "DELETE FROM `{$GLOBALS['DUPX_AC']->wp_tableprefix}options` WHERE `option_name` LIKE ('_transient%') OR `option_name` LIKE ('_site_transient%')");
	...
	foreach ($GLOBALS['DUPX_AC']->opts_delete as $value) {
		mysqli_query($dbh, "DELETE FROM `{$GLOBALS['DUPX_AC']->wp_tableprefix}options` WHERE `option_name` = '{$value}'");
	}
