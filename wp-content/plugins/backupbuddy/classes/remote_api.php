...
class backupbuddy_remote_api {
	...
	public static function localCall( $keySet = false, $importbuddy = false ) {
		...
		if ( true !== $importbuddy ) {
			$functionName = '_verb_' . backupbuddy_core::getHttpHeader( 'backupbuddy_verb' );
		} else {
			$functionName = '_verb_importbuddy_' . backupbuddy_core::getHttpHeader( 'backupbuddy_verb' );
		}
		
		// Does verb exist?
		if ( false === method_exists( 'backupbuddy_remote_api', $functionName ) ) {
			$message = 'Error #843489974: Unknown verb `' . backupbuddy_core::getHttpHeader( 'backupbuddy_verb' ) . '`.';
			pb_backupbuddy::status( 'error', $message );
			self::_reply( array( 'success' => false, 'error' => $message ) );
		} else {
	...
	public static function remoteCall( $remoteAPI, $verb, $moreParams = array(), $timeout, $files = array(), $returnRaw = false ) {
		...
		$response = wp_remote_post( $remoteAPI['siteurl'], array(
				...
				'headers' => array(
						...
						'backupbuddy_verb' => $verb,
						'backupbuddy_now' => $now,
						'backupbuddy_signature' => $signature,
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( false === ( $return = @unserialize( $response['body'] ) ) ) {
				return self::_error( 'Error #8001: Unable to decode Deployment response. <b>Things to check:</b><br><ol><li>Verify both sites are running the latest BackupBuddy version (v8.2 introduced non-backward-compatible changes)</li><li> Check the remote site API URL is correct: ' . $remoteAPI['siteurl'] . '</li><li>If you changed the remote site API key you must update it into this site</li><li>Make sure the remote site has the API enabled in its wp-config.php by adding <i>define( \'BACKUPBUDDY_API_ENABLE\', true );</i> somewhere ABOVE the line "That\'s all, stop editing!".</li></ol>Return data: `' . $sanibody . '`.' );
			} else {
	...
	private static function _verb_renderImportBuddy() {
		...
		$password = md5( md5( backupbuddy_core::getHttpHeader( 'backupbuddy_api_key' ) ) );
	...
	public static function init_incoming_call() {
		$key_public = backupbuddy_core::getHttpHeader( 'backupbuddy_api_key' );
		$verb = backupbuddy_core::getHttpHeader( 'backupbuddy_verb' );
		$time = backupbuddy_core::getHttpHeader( 'backupbuddy_now' );
		$signature = backupbuddy_core::getHttpHeader( 'backupbuddy_signature' );
